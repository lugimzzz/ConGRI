...
def weigth_init(m):
    if isinstance(m, nn.Conv2d):
        ...
        # m.weight.data.uniform_(0.0, 0.05)
        # m.bias.data.fill_(0)
        n = m.in_features
        y = 1.0 / np.sqrt(n)
...
def second_model(train_features, valid_features, test_features, fold):
    ...
    for epoch in range(0, Config.train_number_epochs):
        ...
        for phase in ['train', 'valid', 'test']:
            ...
            print('{} loss: {:.4f} acc: {:.4f} tpr: {:.4f} fpr: {:.4f} f1: {:.4f}'.format(phase, avg_loss[phase], acc,
                                                                                          tpr, fpr, f1))
            if phase == 'test':
...
def second_model_test(test_features, fold):
    ...
    model.load_state_dict(torch.load('./result/{}.pkl'.format(weight)))
    model.eval()
    cross_entropy_loss = nn.BCELoss()
    test_set = LoadFeatureSet(test_features)
    ...
    for i, data in enumerate(dataloaders, 0):
        ...
        for ii in range(output.shape[0]):
            all_de.append([output[ii], label[ii]])
            if label[ii] == 0:
                dist_0.append(output[ii])
            else:
                dist_1.append(output[ii])
    dist_0 = np.array(dist_0)
    dist_1 = np.array(dist_1)
    np.savez('./dist/dist_0_{}.npz'.format(num), embed=np.array(dist_0))
    np.savez('./dist/dist_1_{}.npz'.format(num), embed=np.array(dist_1))
    ...
    for th in [0.5]:
        ...
        print('test  loss: {:.4f} acc: {:.4f} recall: {:.4f} pre: {:.4f} f1: {:.4f}'.format(avg_loss, acc, recall,
                                                                                            precision, f1))
        print(tp, fp, tn, fn)
...
def main():
    ...
    # train_features = np.load('./{}/train.npz'.format(num), allow_pickle=True)
    # valid_features = np.load('./{}/valid.npz'.format(num), allow_pickle=True)

    test_features = np.load('./{}/test.npz'.format(num), allow_pickle=True)
    print('data loaded')
